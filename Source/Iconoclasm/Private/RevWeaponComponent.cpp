...
URevWeaponComponent::URevWeaponComponent()
{
	// Default offset from the character location for projectiles to spawn
	MuzzleOffset = FVector(100.0f, 0.0f, 10.0f);
	CurrentWeaponMode = ERevMode::RevMode1;
}

void URevWeaponComponent::BeginPlay()
{
	Super::BeginPlay();
	Character = Cast<AIconoclasmCharacter>(GetOwner());
}

void URevWeaponComponent::AttachWeapon(AIconoclasmCharacter* TargetCharacter)
{
	...
	if (APlayerController* PlayerController = Cast<APlayerController>(Character->GetController()))
	{
		...
		{
			// Fire
			EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Triggered, this, &URevWeaponComponent::Fire);
			EnhancedInputComponent->BindAction(AltFireAction, ETriggerEvent::Triggered, this, &URevWeaponComponent::AltFire);
			EnhancedInputComponent->BindAction(SwitchFireModeAction, ETriggerEvent::Triggered, this, &URevWeaponComponent::SwitchFireMode);
		}
	}
}

void URevWeaponComponent::Fire()
{
	...
	{
	case ERevMode::RevMode1:
		GunslingerMode();
		break;
	case ERevMode::RevMode2:
		HellfireMode();
		break;
	default:
		break;
...
void URevWeaponComponent::AltFire()
{
	if (Character == nullptr || Character->GetController() == nullptr)
	{
		return;
...
void URevWeaponComponent::SwitchFireMode()
{
	// Cycle through the weapon modes
	CurrentWeaponMode = static_cast<ERevMode>((static_cast<uint8>(CurrentWeaponMode) + 1) % (static_cast<uint8>(ERevMode::RevMode2) + 1));
}

void URevWeaponComponent::PerformHitscan(FVector& ImpactLocation)
{
...
void URevWeaponComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void URevWeaponComponent::GunslingerMode()
{
...
void URevWeaponComponent::AltGunslingerMode()
{
...
void URevWeaponComponent::HellfireMode()
{
